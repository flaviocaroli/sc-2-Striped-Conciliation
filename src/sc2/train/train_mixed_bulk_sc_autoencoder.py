...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a mixed bulk/sc autoencoder on the shared gene space.")
...
@torch.no_grad()
def eval_loader(model: nn.Module, loader: DataLoader, device: torch.device) -> float:
    model.eval()
...
def main() -> None:
    ...
    model = BulkAutoencoder(
        input_dim=bulk_train.n_features,
    ...
    bulk_weight = float(train_section["bulk_loss_weight"])
    sc_weight = float(train_section["sc_loss_weight"])
    ...
    for epoch in range(1, int(train_section["epochs"]) + 1):
        ...
        n_steps = max(len(bulk_train_loader), len(sc_train_loader))
        ...
        for _ in range(n_steps):
            ...
            loss = bulk_weight * loss_b + sc_weight * loss_s
        ...
        val_bulk_loss = eval_loader(model, bulk_val_loader, device)
        val_sc_loss = eval_loader(model, sc_val_loader, device)
        val_total = bulk_weight * val_bulk_loss + sc_weight * val_sc_loss
    ...
    test_bulk_loss = eval_loader(model, bulk_test_loader, device)
    test_sc_loss = eval_loader(model, sc_test_loader, device)
    test_total = bulk_weight * test_bulk_loss + sc_weight * test_sc_loss
