...
def prepare_model(
    ...
    if model_name == "scprint1":
        ...
        def infer(
            ...
            if (
                "source_row"
                not in processed.obs.columns
            ):
                raise RuntimeError(
                    "source_row was lost during native preprocessing"
                )
            ...
            if (
                source_rows.ndim != 1
                or source_rows.size < 100
                or source_rows.size > 5000
                or np.unique(
                    source_rows
                ).size
                != source_rows.size
                ...
                raise RuntimeError(
                    "invalid source_row mapping after preprocessing"
                )
            ...
            if (
                ...
                raise RuntimeError(
                    "prediction/preprocessed cell-count mismatch"
                )
    ...
    if model_name == "scprint2":
        ...
        expected = int(
            info[
                "model_supported_p3_overlap"
            ]
        )

        if len(fixed) != expected:
            raise RuntimeError(
                "scPRINT-2 model-supported P3 overlap changed: "
                f"{len(fixed)} != {expected}"
            )
...
def main():
    ...
    if (
        ...
        raise RuntimeError(
            "amendment identity/status mismatch"
        )

    expected_amendment_sha = os.environ.get(
    ...
    if (
        ...
        raise RuntimeError(
            "amendment/base-protocol mismatch"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "scPRINT-2 completion task mismatch"
        )
    ...
    if model_name != "scprint2":
        raise RuntimeError(
            "completion runner is scPRINT-2 only"
        )
    ...
    try:
        ...
        if potential_idx.size < 10000:
            raise RuntimeError(
                "insufficient potential P3 coverage: "
                f"{potential_idx.size}"
            )

        potential_gene_order_sha = (
            hashlib.sha256(
                "\n".join(
                    potential_genes.tolist()
                ).encode()
            )
            .hexdigest()
        )
        ...
        for item in records:
            ...
            if len(
                ...
                raise RuntimeError(
                    "insufficient native returned P3 coverage: "
                    f"{len(evaluated_genes)}"
                )

            evaluated_idx = np.asarray(
                [
                    p3_index[
                        gene
                    ]
            ...
            if (
                ...
                raise RuntimeError(
                    "prediction/source_row size mismatch"
                )
            ...
            corrupted_eval = corrupted[
                source_rows
            ]

            positive_eval = positive[
                source_rows
            ]

            true_zero_eval = true_zero[
                source_rows
            ]
            ...
            if not (
                prediction.shape
                == target_eval.shape
                == corrupted_eval.shape
                == positive_eval.shape
                == true_zero_eval.shape
            ):
                raise RuntimeError(
                    "paired evaluation-array shape mismatch"
                )
            ...
            payload = {
                ...
                "native_gene_coverage_fraction_of_potential":
                    float(
                        len(
                            evaluated_genes
                        )
